...
import type { CreateCoordinatorDto, UpdateCoordinatorDto } from './coordinator.schema';
import { AppError } from '../../shared/errors/AppError';
import { sendWelcomeEmail } from '../../shared/utils/email';

export class CoordinatorService {
  constructor(private repository: CoordinatorRepository) {}

  async getAll() {
    const coordinators = await this.repository.findAllActive();
  ...
  async delete(id: number) {
    const user = await this.repository.findById(id);
    if (!user || user.role !== 'ACADEMIC_COORDINATOR' || !user.isActive) {
      throw new AppError('Coordinator not found', 404);
    }
